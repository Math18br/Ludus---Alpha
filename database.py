...
def atualizar_diario_classe(data):
    query = """
    SELECT 
        string_agg(DISTINCT justificativa, ', ' ORDER BY justificativa) AS justificativas,
        string_agg(DISTINCT observacoes, ', ' ORDER BY observacoes) AS observacoes 
    FROM 
        frequencia 
    WHERE 
        data_presenca = %s
    ...
    if resultados and resultados[0]:
        ...
        # Remover valores vazios nas justificativas e observações
        justificativas = ", ".join([j for j in (justificativas or "").split(", ") if j])
        observacoes = ", ".join([o for o in (observacoes or "").split(", ") if o])

        # Verificar se está vazio após o filtro e ajustar a mensagem
        if not justificativas:
            justificativas = "Não há Justificativas no dia selecionado"
        if not observacoes:
            observacoes = "Não há Observações no dia selecionado"
